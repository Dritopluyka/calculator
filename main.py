...
def decor(func):
    def wrapper(*args, **kwargs):   #написать декоратор (Леся)
...
def calc(counter, expression):
    while counter <= len(expression):
        try:
            ...
            elif expression[counter] == "+": # тут должна быть функция для сложения (Максим)

            elif expression[counter] == "*": # тут должна быть функция для умножения (Паша)

            elif expression[counter] == "/": # тут должна быть функция для деления (Андрей)
